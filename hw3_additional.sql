DROP DATABASE IF EXISTS hw3_additionally;
CREATE DATABASE hw3_additionally;
USE hw3_additionally;
...
-- 1. Посчитаем количество документов у каждого пользователя (doc, docx, html):

SELECT u.id, u.firstname, COUNT(m.filename) as document_count
FROM users u
JOIN media m ON u.id = m.user_id
WHERE m.media_type_id IN (4)
GROUP BY u.id, u.firstname;

-- 2.  Посчитаем лайки для документов, которые выложил каждый пользователь (моих медиа)
